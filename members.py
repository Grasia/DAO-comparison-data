...
if __name__ == '__main__':
    ...
    daostack_main: pd.DataFrame = pd.read_csv(
        os.path.join(data_path, 'daostack_total_users_mainnet.csv'),
        header=0)
    daohaus_main: pd.DataFrame = pd.read_csv(
        os.path.join(data_path, 'daohaus_total_users_mainnet.csv'),
        header=0)
    daostack_xdai: pd.DataFrame = pd.read_csv(
        os.path.join(data_path, 'daostack_total_users.csv'),
        header=0)
    daohaus_xdai: pd.DataFrame = pd.read_csv(
        os.path.join(data_path, 'daohaus_total_users.csv'),
        header=0)
    ...
    update_layout(fig, dates=daohaus_main['createdAt'])
    fig.show()
